        ...
        if self.method == 'item' and hasattr(self, 'similarity_matrix'):
            ...
            for i, movie_id1 in enumerate(movie_ids):
                for movie_id2 in movie_ids[i+1:]:
                    # Get similarity from similarity matrix
                    if movie_id1 in self.similarity_matrix and movie_id2 in self.similarity_matrix[movie_id1]:
                        sim = self.similarity_matrix[movie_id1][movie_id2]
                    elif movie_id2 in self.similarity_matrix and movie_id1 in self.similarity_matrix[movie_id2]:
                        sim = self.similarity_matrix[movie_id2][movie_id1]
                    else:
                        sim = 0.0
                    if sim >= self.similarity_threshold:
